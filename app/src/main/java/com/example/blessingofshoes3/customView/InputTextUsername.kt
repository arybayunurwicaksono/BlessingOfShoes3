...
class InputTextUsername: AppCompatEditText {
    ...
    private fun init() {
        drawable =
            ContextCompat.getDrawable(context, R.drawable.ic_baseline_assignment_24) as Drawable
        ...
        addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                if (!s.isNullOrEmpty() && s.length < 6)
                    error = context.getString(R.string.username_required)
            }
        })
    }
}